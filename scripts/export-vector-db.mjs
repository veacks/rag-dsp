...
CREATE VIRTUAL TABLE IF NOT EXISTS rag_vec USING vec0(embedding float[${dim}]);
\`);
  return db;
}
...
async function importRows(rowsUrl) {
  ...
  try {
    ...
    const chunkStmt = await db.prepare("INSERT INTO rag_chunks(rowid, chunk_id, text, metadata_json, source, path, url) VALUES(?, ?, ?, ?, ?, ?, ?)");
    let rowid = 1;
    for (const line of lines) {
      const row = JSON.parse(line);
      const metadata = row.metadata || {};
      chunkStmt.run([rowid, row.id, row.text || "", JSON.stringify(metadata), metadata.source || null, metadata.path || null, metadata.url || null]);
      const vecStmt = await db.prepare(\`INSERT INTO rag_vec(rowid, embedding) VALUES(\${rowid}, ?)\`);
      vecStmt.run([vectorBuffer(row.embedding)]);
      rowid += 1;
    }
...
async function handleMessage(data) {
  ...
  try {
    if (type === "init") {
      ...
      return { id, ok: true, result: { ready: true, sqliteVec: versionStmt.get({})?.version, cachedRows: await rowCount() } };
    }
...
async function writeSqliteExport(outDir, chunks, manifest) {
...
`;
  ...
  const sqlitePath = path.join(sqliteDir, 'rag.sqlite');
...
async function main() {
  ...
  if (sqlite) {
    const sqliteDir = await writeSqliteExport(outDir, chunks, manifest);
